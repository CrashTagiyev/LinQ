...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        new Debtor("Richard M. Thomas", DateTime.Parse("July 11, 1972"), "510-735-3359", "RichardMThomas@jourrapide.com", "4454 Green Avenue Oakland, CA 94609", 787),
        new Debtor("Richard M. Thomas", DateTime.Parse("July 11, 1972"), "510-735-3359", "RichardMThomas@jourrapide.com", "4454 Green Avenue Oakland, CA 94609", 787),
        new Debtor("Richard M. Thomas", DateTime.Parse("July 11, 1972"), "510-735-3359", "RichardMThomas@jourrapide.com", "4454 Green Avenue Oakland, CA 94609", 787),
        new Debtor("Richard M. Thomas", DateTime.Parse("July 11, 1972"), "510-735-3359", "RichardMThomas@jourrapide.com", "4454 Green Avenue Oakland, CA 94609", 787),
        new Debtor("Richard M. Thomas", DateTime.Parse("July 11, 1972"), "510-735-3359", "RichardMThomas@jourrapide.com", "4454 Green Avenue Oakland, CA 94609", 787),
        ...
        new Debtor("Pamela H. Beauchamp", DateTime.Parse("November 20, 1959"), "801-559-6347", "PamelaHBeauchamp@jourrapide.com", "3239 Tori Lane Salt Lake City, UT 84104", 999999),
        new Debtor("samil eliyev", DateTime.Parse("November 20, 1959"), "123-456-7890", "PamelaHBeauchamp@jourrapide.com", "3239 Tori Lane Salt Lake City, UT 84104", 999999)
    };
        //1)
        var debtorQuerry = debtors.Where(debtor => debtor.Email.EndsWith("rhyta.com") || debtor.Email.EndsWith("dayrep.com")).ToList();
        //debtorQuerry.ForEach(ac => Console.WriteLine(ac));
        //2)
        debtorQuerry = debtors.Where(debtor => debtor.GetAge() >= 26 && debtor.GetAge() <= 36).ToList();
        //debtorQuerry.ForEach(ac => Console.WriteLine(ac.GetAge()));
        //3)
        debtorQuerry = debtors.Where(debtor => debtor.Debt <= 5000).ToList();
        //debtorQuerry.ForEach(ac => Console.WriteLine(ac));
        //6)
        debtorQuerry = debtors.Where(debtor => debtor.FullName.Length >= 18 && debtor.Phone.Count(count => count == '7') >= 2).ToList();
        //debtorQuerry.ForEach(ac => Console.WriteLine(ac));
        //7)
        debtorQuerry = debtors.Where(debtor => debtor.BirthDay.Month == 1 || debtor.BirthDay.Month == 2 || debtor.BirthDay.Month == 12).ToList();
        //debtorQuerry.ForEach(ac => Console.WriteLine(ac));
        //8)
        debtorQuerry = debtors.Where(debtor => debtor.Debt <= debtors.Sum(debtor => debtor.Debt) / debtors.Count).ToList();
        //debtorQuerry.ForEach(ac => Console.WriteLine(ac));
        //9)
        debtorQuerry = debtors.Where(debtor => debtor.Phone.Count(any => any == '8') == 0).ToList();
        //debtorQuerry.ForEach(ac => Console.WriteLine(ac.FullName + " " + ac.GetAge() + " " + ac.Debt));
        //10)

        debtorQuerry = debtors.Where(debtor => HasDuplicateChars(debtor.FullName) == true).ToList().OrderBy(sort => sort.FullName).ToList();
        //debtorQuerry.ForEach(ac => Console.WriteLine(ac));

        //11)

        var most = debtors.GroupBy(debtor => debtor.BirthDay.Year).ToList().OrderByDescending(order => order.Count()).First().ToList()[0];

        //12)
        debtorQuerry = debtors.OrderByDescending(debtor => debtor.Debt).ToList();
        //debtorQuerry.GetRange(0, 5).ForEach(item => Console.WriteLine(item));

        //13)

        int allDepts = debtors.Sum(sum => sum.Debt);
        //Console.WriteLine(allDepts);

        //14)
        var secondWorldWarDeptors = debtorQuerry.Where(debtor => debtor.BirthDay.Year <= 1945).ToList();
        //secondWorldWarDeptors.ForEach(ac => Console.WriteLine(ac));

        //15)   
        debtorQuerry = debtors.Where(debtor => HasDuplicateNumbers(debtor.Phone) == false).ToList();
        //debtorQuerry.ForEach(ac => Console.WriteLine(ac));

        //16)
        debtorQuerry = debtors.Where(debtor => CalculateMonths(debtor)).ToList();
        debtorQuerry.ForEach(cw => Console.WriteLine(cw));
        //17)
        debtorQuerry = debtors.Where(debtor => HasWord(debtor.FullName, "smile")).ToList();
        //debtorQuerry.ForEach(cw => Console.WriteLine(cw));

    }
    public static bool CalculateMonths(Debtor debtor)
    {
        DateTime Temp = new DateTime(DateTime.Now.Year, debtor.BirthDay.Month, debtor.BirthDay.Day);
        if ((Temp.Month - DateTime.Now.Month >= 1))
        {
            return 500 * (Temp.Month - DateTime.Now.Month) >= debtor.Debt;

        }
        return false;
    }

    public static bool HasWord(string input, string writeWordToFind)
    {
        bool prove = false;
        for (int i = 0; i < writeWordToFind.Length; i++)
        {
            if (input.Contains(writeWordToFind[i]))
            {
                prove = true;
            }
            else return false;
    ...
    {
        return input.GroupBy(c => c).Any(group => group.Count() > 1);
    }
    public static bool HasDuplicateNumbers(string input)
    {
        ...
        return temp.GroupBy(c => c).Any(group => group.Count() > 1);
    }
}
